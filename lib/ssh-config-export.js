...
export function buildSshConfigText(targets, options = {}) {
    ...
    if (defaultUser || hostDefaultEntries.length > 0 || shouldWriteGlobalKeyPath) {
        lines.push('Host *');
        if (defaultUser) {
        ...
        if (shouldWriteGlobalKeyPath) {
            lines.push(`  IdentityFile ${defaultKeyPath}`);
        }
        lines.push('');
    }

    (Array.isArray(targets) ? targets : []).forEach((target) => {
        const hostOverride = hostOverrides[target.alias] || {};
        const selectedUser = hostOverride.user || userOverrides[target.alias] || '';
        const selectedKeyPath = normalizeSshKeyPath(hostOverride.keyPath);
        ...
        if (selectedKeyPath) {
            if (hasIdentityFileDefault || !shouldWriteGlobalKeyPath || selectedKeyPath !== defaultKeyPath) {
